...
func (c *WSClient) Start() {
	conn, resp, err := websocket.DefaultDialer.Dial("ws://127.0.0.1:8002/ws", http.Header{})
	if err != nil {
...
func (c *WSClient) HandlePackage(bytes []byte) {
	...
	case pb.PackageType_PT_SYNC:
		...
		fmt.Println("Offline message sync reponse code:", output.Code, "message:", output.Message)
		fmt.Printf("%+v \n", &output)
